package com.sulake.habbo.catalog.purchase
{
   ...
   public class PurchaseConfirmationDialog implements IGetImageListener
   {
      ...
      private var var_92:IPurchasableOffer;
      ...
      private const const_2001:int = 10;
      ...
      private const const_1708:uint = 4.293848814E9;
      
      private const const_1709:uint = 4.294967295E9;
      
      private const const_2000:uint = 4.291613146E9;
      ...
      {
         if(this.var_92 == null || this.var_92.offerId != param1.offerId)
         {
            return;
         }
      ...
      private function showConfirmationDialog() : void
      {
         ...
         if(_loc1_ != null)
         {
            _loc3_ = this._catalog.getProductData(this.var_92.localizationId);
         ...
         if(_loc2_ != null)
         {
            _loc4_ = this.var_92.page.viewer.roomEngine;
            _loc7_ = 0;
            _loc8_ = "";
            _loc9_ = "";
            if(this.var_92 is Offer)
            {
               _loc10_ = this.var_92.productContainer.firstProduct;
               _loc9_ = _loc10_.productType;
               _loc7_ = _loc10_.productClassId;
               _loc8_ = _loc10_.extraParam;
            }
            else if(this.var_92 is ClubBuyOfferData)
            {
               _loc9_ = "null";
            }
            switch(_loc9_)
      ...
      {
         var _loc1_:IProductData = this._catalog.getProductData(this.var_92.localizationId);
         ...
         var _loc3_:BalanceAndCost = new BalanceAndCost(this._catalog,this._localization,this.var_92);
      ...
      private function showGiftDialog() : void
      {
         ...
         if(_loc1_.isEnabled)
         {
            ...
            this.var_1556 = _loc1_.stuffTypes;
            this.var_707 = _loc1_.boxTypes;
            this.var_706 = _loc1_.ribbonTypes;
            this.var_1084 = this.var_1556[0];
            this.var_340 = this.var_707[0];
            this.var_338 = this.var_706[0];
            this.initColorGrid();
      ...
      private function updatePreview() : void
      {
         ...
         if(this.var_338 < 0)
         {
            this.var_338 = this.var_706.length - 1;
         }
         if(this.var_338 > this.var_706.length - 1)
         {
            this.var_338 = 0;
         }
         if(this.var_340 < 0)
         {
            this.var_340 = this.var_707.length - 1;
         }
         if(this.var_340 > this.var_707.length - 1)
         {
            this.var_340 = 0;
         }
         var _loc1_:int = this.var_707[this.var_340] * 1000 + this.var_706[this.var_338];
         if(this._window == null)
         {
            return;
         }
         var _loc2_:IRoomEngine = this.var_92.page.viewer.roomEngine;
         _loc3_ = _loc2_.getFurnitureImage(this.var_1084,new Vector3d(180),64,this,0,_loc1_.toString());
         if(_loc3_ == null)
         {
            return;
         }
         this.var_1345 = _loc3_.id;
      ...
      private function initColorGrid() : void
      {
         ...
         var _loc3_:Array = this.var_1556;
      ...
      private function giveGift() : void
      {
         ...
         var _loc7_:int = !!_loc6_ ? 0 : int(this.var_1084);
         var _loc8_:int = !!_loc6_ ? 0 : int(this.var_707[this.var_340]);
         var _loc9_:int = !!_loc6_ ? 0 : int(this.var_706[this.var_338]);
         this._catalog.purchaseProductAsGift(this._pageId,this.var_92,this.var_1346,_loc2_,_loc4_,_loc7_,_loc8_,_loc9_);
      }
      ...
      private function confirmationHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "gift_button":
               ...
               break;
            case "buy_button":
               this._catalog.purchaseProduct(this._pageId,this.var_92.offerId,this.var_1346);
               this.dispose();
               break;
            case "cancel_button":
      ...
      private function nameInputHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param1.type)
         {
            case WindowEvent.const_105:
               _loc3_ = param1.window as ITextFieldWindow;
               if(_loc3_ == null)
               {
                  return;
               }
               if(this.var_2347 == _loc3_.text)
               {
                  return;
               }
               ...
               for each(_loc7_ in this.var_224)
               {
                  ...
                  if(_loc5_.length >= this.const_2001)
                  {
                     break;
                  }
               }
               this.updateSuggestions(_loc5_);
               this.var_2347 = _loc3_.text;
               break;
            case WindowMouseEvent.const_45:
               ...
               break;
            case WindowKeyboardEvent.const_342:
               _loc6_ = param1 as WindowKeyboardEvent;
               switch(_loc6_.keyCode)
               {
                  case Keyboard.UP:
                     this.highlightSuggestion(this.var_339 - 1);
                     break;
                  case Keyboard.DOWN:
                     this.highlightSuggestion(this.var_339 + 1);
                     break;
      ...
      private function selectHighlighted() : void
      {
         ...
         var _loc2_:IWindowContainer = _loc1_.getListItemAt(this.var_339) as IWindowContainer;
         if(_loc2_ == null)
         {
            return;
         }
      ...
      private function updateSuggestions(param1:Array) : void
      {
         ...
         if(this.var_577 == null)
         {
            this.var_577 = this.createWindow("suggestion_list_item") as IWindowContainer;
            this.var_577.procedure = this.suggestionsEventHandler;
         }
         if(this.var_251 == null || this.var_577 == null)
         {
            return;
         }
         ...
         {
            _loc5_ = this.var_577.clone() as IWindowContainer;
            if(_loc5_ != null)
            {
      ...
      private function highlightSuggestion(param1:int) : void
      {
         ...
         _loc3_ = _loc2_.getListItemAt(this.var_339) as IWindowContainer;
         if(_loc3_ != null)
         {
            _loc3_.color = this.getColor(this.var_339);
         }
         this.var_339 = param1;
         if(this.var_339 < 0)
         {
            this.var_339 = _loc2_.numListItems - 1;
         }
         if(this.var_339 >= _loc2_.numListItems)
         {
            this.var_339 = 0;
         }
         _loc3_ = _loc2_.getListItemAt(this.var_339) as IWindowContainer;
         if(_loc3_ != null)
         {
            _loc3_.color = this.const_2000;
         }
      }
      
      private function getColor(param1:int) : uint
      {
         return param1 % 2 == 0 ? uint(this.const_1708) : uint(this.const_1709);
      }
      ...
      private function updateColorGrid() : void
      {
         ...
         while(_loc4_ < _loc1_.numGridItems)
         {
            ...
            if(_loc2_ != null)
            {
               ...
               if(_loc3_ != null)
               {
                  _loc3_.visible = _loc2_.id == this.var_1084;
               }
            }
            _loc4_++;
         }
