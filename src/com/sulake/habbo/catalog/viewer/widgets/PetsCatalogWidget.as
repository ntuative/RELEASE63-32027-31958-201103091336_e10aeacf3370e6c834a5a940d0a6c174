package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : Boolean
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_105,this.onTextWindowEvent);
         ...
         this.var_493 = this.getPetTypeIndexFromProduct(_loc3_.localizationId);
         if(this.var_493 >= 8)
         {
            return false;
         }
         this.updateAvailableBreeds(_loc3_.localizationId);
         this.var_2608 = _loc3_.localizationId;
         this.var_372 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc5_ = _loc4_.avatarRenderManager.petDataManager.getPetData(this.var_493,this);
               if(_loc5_)
               {
         ...
         this._offers.add(this.var_493,_loc3_);
         events.addEventListener(WidgetEvent.CWE_SELECT_PRODUCT,this.onSelectProduct);
         events.addEventListener(WidgetEvent.CWE_COLOUR_INDEX,this.onColourIndex);
         events.addEventListener(WidgetEvent.const_628,this.onApproveNameResult);
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_500 = param1.index;
         if(this.var_500 < 0 || this.var_500 > this.var_372.length)
         {
            this.var_500 = 0;
         }
         this.updateImage();
      ...
      {
         if(param1.productCode == this.var_2608)
         {
      ...
      private function parseSellableBreeds(param1:Array) : Array
      {
         ...
         {
            if(_loc3_.type == this.var_493 && _loc3_.sellable)
            {
      ...
      private function updateBreedSelections() : void
      {
         ...
         while(_loc3_ < this.var_201.length)
         {
            ...
            _loc2_.push(_loc1_.getKey(this.getRaceLocalizationKey(this.var_493,_loc5_),this.getRaceLocalizationKey(this.var_493,_loc5_)));
            _loc3_++;
         }
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_201 == null || this.var_765 >= this.var_201.length)
         {
            return;
         }
         var _loc2_:* = 0;
         if(this.var_500 >= 0 && this.var_500 < this.var_372.length)
         {
            _loc7_ = this.var_372[this.var_500] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:int = this.var_201[this.var_765] as int;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_493,_loc3_,_loc2_,AvatarScaleType.const_57,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_308,AvatarAction.const_903);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_500 >= this.var_372.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_372[this.var_500] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:int = this.var_201[this.var_765] as int;
