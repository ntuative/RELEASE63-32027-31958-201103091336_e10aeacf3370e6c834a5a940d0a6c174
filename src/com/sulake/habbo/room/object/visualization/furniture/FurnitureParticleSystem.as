package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1156:Number = 1.0;
      
      private var var_612:BitmapData;
      
      private var var_761:ColorTransform;
      
      private var var_1445:ColorTransform;
      
      private var var_1833:Matrix;
      
      private var _translationMatrix:Matrix;
      
      private var var_763:Number = 1.0;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_761 = new ColorTransform();
         this.var_761.alphaMultiplier = 1;
         this.var_1445 = new ColorTransform();
         this.var_1833 = new Matrix();
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in this.var_1157)
         {
            _loc1_.dispose();
         }
         this.var_1157 = null;
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_96 = this.var_1157[param1];
         this.var_949 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1834 >= 0)
         {
            this.var_266 = this._visualization.getSprite(this.var_1834);
            if(this.var_266 && this.var_266.asset)
            {
               ...
               if(this.var_86 == null)
               {
                  this.var_86 = this.var_266.asset.clone();
                  if(this.var_761.alphaMultiplier != 1)
                  {
                     this.var_612 = new BitmapData(this.var_86.width,this.var_86.height,true,4278190080);
                  }
               }
               this.var_2122 = -this.var_266.offsetX;
               this.var_2121 = -this.var_266.offsetY;
               this.var_266.asset = this.var_86;
            }
            if(this.var_86)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_96.y * this.var_1156;
         }
         return 0;
      }
      ...
      {
         if(!this.var_96 || !this.var_266)
         {
            return;
         }
         if(this.var_86 && this.var_266.asset != this.var_86)
         {
            this.var_266.asset = this.var_86;
         }
         if(this.var_949)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_949 && this.var_96.hasIgnited)
         {
            this.var_949 = true;
         }
         _loc8_ = this.var_1118 * this.var_1156;
         this.var_96.update();
         if(this.var_949)
         {
            if(this.var_96.roomObjectSpriteId >= 0)
            ...
            {
               this.var_86.draw(this.var_612,this.var_1833,this.var_761,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_96.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2122 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1156;
               _loc4_ = this.var_2121 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1156;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1445.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_86.draw(_loc7_,this._translationMatrix,this.var_1445,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2613 = parseInt(param1.@size);
         this.var_1834 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1118 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1156 = this.var_2613 / 64;
         this.var_763 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_763 = Math.min(this.var_763,1);
         this.var_761.alphaMultiplier = this.var_763;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "FurnitureEcotronBoxLogic" ? false : true;
