package com.sulake.habbo.sound.music
{
   ...
   public class HabboMusicController implements IHabboMusicController, IDisposable
   {
      ...
      public static const const_1535:int = -1;
       
      
      private var var_487:HabboSoundManagerFlash10;
      ...
      private var var_868:Map;
      
      private var var_870:Map;
      
      private var var_1697:RoomItemPlayListController = null;
      ...
      public function HabboMusicController(param1:HabboSoundManagerFlash10, param2:IEventDispatcher, param3:IEventDispatcher, param4:IConnection)
      {
         ...
         this.var_868 = new Map();
         this.var_870 = new Map();
         this.var_194 = new Map();
         this.var_869 = HabboMusicPrioritiesEnum.const_645;
         super();
         this.var_487 = param1;
         this._events = param2;
         this.var_82 = param3;
         this._connection = param4;
         this.var_1697 = new RoomItemPlayListController(param1,this,param2,param3,param4);
         this._connection.addMessageEvent(new TraxSongInfoMessageEvent(this.onSongInfoMessage));
         this.var_1086 = new Timer(1000);
         this.var_1086.start();
         this.var_1086.addEventListener(TimerEvent.TIMER,this.sendNextSongRequestMessage);
      }
      ...
      {
         return this.var_1697 as IPlayListController;
      }
      ...
      public function playSong(param1:int, param2:int, param3:Number = 0) : Boolean
      {
         ...
         var _loc4_:SongDataEntry = this.var_173.getValue(param1) as SongDataEntry;
         if(_loc4_ == null)
         {
            this.requestSongInfo(param1);
            _loc6_ = new Date();
            this.var_868.add(param1,_loc6_.getTime());
            this.var_870.add(param1,param3);
            return false;
         }
         this.var_870[param1] = param3;
         if(_loc4_.soundObject == null)
         {
            this.var_868[param1] = _loc6_.getTime();
            this.var_870[param1] = param3;
            _loc4_.soundObject = this.var_487.loadTraxSong(_loc4_.id,_loc4_.songData);
         }
         ...
         if(param2 >= this.var_869)
         {
            for each(_loc7_ in this.var_194.getValues())
            {
               if(_loc7_ != param1)
               {
                  _loc8_ = this.var_173.getValue(_loc7_) as SongDataEntry;
      ...
      {
         var _loc4_:int = 0;
         var _loc2_:SongDataEntry = this.var_173.getValue(param1) as SongDataEntry;
         if(_loc2_ != null)
         {
         ...
         if(_loc3_ != -1)
         {
            ...
            this.var_869 = this.getTopPriority();
         }
      }
      ...
      public function stop(param1:int = 10) : void
      {
         ...
         while(_loc2_ < this.var_194.length)
         {
            ...
            if(_loc3_ <= param1)
            {
               ...
               _loc5_ = this.var_173.getValue(_loc4_) as SongDataEntry;
               if(_loc5_ != null)
               {
      ...
      public function updateVolume(param1:Number) : void
      {
         ...
         {
            _loc3_ = this.var_173.getValue(_loc2_) as SongDataEntry;
            if(_loc3_ != null)
            {
      ...
      {
         var _loc2_:int = this.var_194[this.var_869];
         Logger.log("Song loaded : " + param1);
         if(param1 == _loc2_)
         {
            this.playSongObject(param1,this.var_870[param1]);
         }
      }
      ...
      {
         return this.var_173.getValue(param1);
      }
      ...
      private function sendNextSongRequestMessage(param1:TimerEvent) : void
      {
         ...
         var _loc2_:int = this.var_395[0];
         if(this.var_173.getValue(_loc2_) != null)
         {
            this.var_395.pop();
            return;
         }
      ...
      private function onSongInfoMessage(param1:IMessageEvent) : void
      {
         ...
         var _loc4_:* = this.var_173.getValue(_loc3_.id) == null;
         var _loc5_:* = this.var_871.indexOf(_loc3_.id) == -1;
         this.var_395.splice(this.var_395.indexOf(_loc3_.id),1);
         if(!_loc5_)
         {
            this.var_871.splice(this.var_871.indexOf(_loc3_.id),1);
         }
         if(_loc4_)
         {
            if(_loc5_)
            {
               _loc6_ = this.var_487.loadTraxSong(_loc3_.id,_loc3_.data);
               if(_loc6_ == null)
               {
                  return;
               }
               _loc7_ = new SongDataEntry(_loc3_.id,_loc3_.length,_loc3_.name,_loc3_.author,_loc6_);
               this.var_173.add(_loc3_.id,_loc7_);
               _loc8_ = this.var_194[this.var_869];
               if(_loc6_.ready && _loc3_.id == _loc8_)
               {
                  this.playSongObject(_loc8_,this.var_870[_loc8_]);
               }
            }
            else
            {
               ...
               this.var_173.add(_loc3_.id,_loc9_);
               this._events.dispatchEvent(new SongInfoReceivedEvent(SongInfoReceivedEvent.const_752,_loc3_.id));
            }
         }
      ...
      private function playSongObject(param1:int, param2:Number = -1) : Boolean
      {
         ...
         var _loc3_:SongDataEntry = this.var_173.getValue(param1) as SongDataEntry;
         if(_loc3_ == null)
         {
         ...
         _loc4_.volume = this.var_487.volume;
         if(param2 != const_1535)
         {
            _loc4_.position = param2;
            if(this.var_868.getKeys().indexOf(param1) != -1)
            {
               _loc6_ = new Date();
               _loc7_ = (_loc6_.getTime() - this.var_868[param1]) / 1000;
               _loc4_.position += _loc7_;
               this.var_868.remove(param1);
            }
         }
         _loc4_.play();
         if(_loc3_.length >= 8000)
         {
            this.var_487.notifyPlayedSong(_loc3_.name,_loc3_.creator);
         }
