package com.sulake.habbo.navigator
{
   ...
   public class RoomPopupCtrl extends PopupCtrl
   {
      ...
      public function RoomPopupCtrl(param1:HabboNavigator, param2:int, param3:int)
      {
         ...
         this.var_2182 = new RoomThumbnailRenderer(param1);
         this.var_1085 = new TagRenderer(param1,hideInstantly);
      }
      ...
      override public function refreshContent(param1:IWindowContainer) : void
      {
         ...
         {
            this.var_175 = IWindowContainer(param1.findChildByName("details_container"));
         }
         this.var_175.visible = true;
         Util.hideChildren(this.var_175);
         this.refreshRoomName(this.var_175,this.var_111);
         if(this.var_111.ownerName != "" && this.var_111.ownerName != "-")
         {
         }
         this.refreshOwnerName();
         this.refreshTextWithCaption("roomctg",this.var_175,!!this.var_111.event ? this.getEventCtg(this.var_111.categoryId) : this.getRoomCtg(this.var_111.categoryId));
         this.refreshRoomDesc(this.var_175,this.var_111);
         this.refreshExtraCont();
         this.refreshInfo(this.var_175,"trading_allowed",this.var_111.allowTrading);
         this.refreshInfo(this.var_175,"doormode_doorbell",this.var_111.doorMode == 1);
         this.refreshInfo(this.var_175,"doormode_password",this.var_111.doorMode == 2);
         Util.moveChildrenToColumn(this.var_175,["roomname","roomctg_cont","roomowner_cont","roomdesc","extra_cont","doormode_doorbell","doormode_password","trading_allowed"],0,0);
         this.var_175.height = Util.getLowestPoint(this.var_175);
      }
      
      private function refreshOwnerName() : void
      {
         this.refreshTextWithCaption("roomowner",this.var_175,this.var_111.ownerName);
         var _loc1_:ITextWindow = ITextWindow(this.var_175.findChildByName("roomowner"));
         if(this.var_111.ownerName != "" && this.var_111.ownerName != "-")
         {
      ...
      {
         var _loc1_:IWindowContainer = IWindowContainer(this.var_175.findChildByName("extra_cont"));
         Util.hideChildren(_loc1_);
         this.var_1085.refreshTags(_loc1_,this.var_111.tags);
         ...
         this.var_2182.refreshThumbnail(_loc1_,this.var_111.thumbnail,false);
      ...
      private function onOwnerName(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(_loc3_ != null)
         {
            navigator.mainViewCtrl.startSearch(Tabs.const_250,Tabs.const_243,_loc3_.text);
            this.hideInstantly();
         }
      }
   }
