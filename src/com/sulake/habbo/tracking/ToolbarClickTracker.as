package com.sulake.habbo.tracking
{
   import com.sulake.habbo.configuration.IHabboConfigurationManager;
   
   public class ToolbarClickTracker
   {
       
      
      private var var_1258:IHabboTracking;
      
      private var var_1605:Boolean = false;
      
      private var var_2089:int = 0;
      
      private var var_1548:int = 0;
      
      public function ToolbarClickTracker(param1:IHabboTracking)
      {
         super();
         this.var_1258 = param1;
      }
      
      public function dispose() : void
      {
         this.var_1258 = null;
      }
      
      public function configure(param1:IHabboConfigurationManager) : void
      {
         this.var_1605 = Boolean(parseInt(param1.getKey("toolbar.tracking.enabled","1")));
         this.var_2089 = parseInt(param1.getKey("toolbar.tracking.max.events","100"));
      }
      
      public function track(param1:String) : void
      {
         if(!this.var_1605)
         {
            return;
         }
         ++this.var_1548;
         if(this.var_1548 <= this.var_2089)
         {
            this.var_1258.track("toolbar",param1);
         }
      }
   }
}
