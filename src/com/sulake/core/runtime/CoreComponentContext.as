package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1344:String = "asset-libraries";
      
      private static const const_1349:String = "library";
      
      private static const const_1342:String = "service-libraries";
      
      private static const const_1341:String = "library";
      
      private static const const_1347:String = "component-libraries";
      
      private static const const_1345:String = "library";
      
      private static const const_1343:String = "error_data";
      
      private static const const_1346:String = "error_cat";
      
      private static const const_1348:String = "error_desc";
       
      
      private var var_362:LibraryLoaderQueue;
      
      private var var_748:IEventDispatcher;
      
      private var var_492:uint;
      
      private var var_363:Function;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_262.push(new Array());
            this.var_607.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_452.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            case Core.const_1163:
               debug("Core; using simple frame update handler");
               this.var_363 = this.simpleFrameUpdateHandler;
               break;
            case Core.const_838:
               debug("Core; using complex frame update handler");
               this.var_363 = this.complexFrameUpdateHandler;
               break;
            case Core.const_904:
               debug("Core; using profiler frame update handler");
               this.var_363 = this.profilerFrameUpdateHandler;
               this.var_200 = new Profiler(this);
               attachComponent(this.var_200,[new IIDProfiler()]);
               var_452.addChild(new ProfilerViewer(this.var_200));
               break;
            case Core.const_568:
               debug("Core; using experimental frame update handler");
               this.var_363 = this.experimentalFrameUpdateHandler;
               break;
            case Core.CORE_SETUP_DEBUG:
               debug("Core; using debug frame update handler");
               this.var_363 = this.debugFrameUpdateHandler;
         }
      }
      ...
      {
         return this.var_492 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            var_452.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
            var_452 = null;
            _loc3_ = 0;
            while(_loc3_ < CoreComponentContext.const_102)
            {
               _loc2_ = this.var_262[_loc3_] as Array;
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1343,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1346,String(param3));
         ErrorReportStorage.setParameter(const_1348,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_748 = loadingEventDelegate;
         if(this.var_362 == null)
         {
            this.var_362 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1344)[0];
            if(node != null)
            {
               list = node.child(const_1349);
         ...
         try
         {
            node = config.child(const_1342)[0];
            if(node != null)
            {
               list = node.child(const_1341);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         try
         {
            node = config.child(const_1347)[0];
            if(node != null)
            {
               list = node.child(const_1345);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_748)
         {
            this.var_748.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_748)
         {
            ...
            this.var_748.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_748)
                  {
                     this.var_748.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_492 - this.var_362.length,this.var_492,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_200 ? 0 : this.var_1429 & 0;
         if(_loc3_ == Core.const_568)
         {
            this.var_262[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_262[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc3_:* = null;
         var _loc6_:* = null;
         var _loc4_:int = !!this.var_200 ? 0 : this.var_1429 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_102)
         {
            _loc3_ = this.var_262[_loc5_] as Array;
            if(_loc4_ == Core.const_568)
            {
      ...
      private function onEnterFrame(param1:Event) : void
      {
         ...
         this.var_363(_loc2_,_loc2_ - this._lastUpdateTimeMs);
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_607[priority] = 0;
            receivers = this.var_262[priority];
            index = 0;
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var msCurrentTime:uint = param1;
         var msSinceLastUpdate:uint = param2;
         var maxTimePerFrame:uint = 1000 / var_452.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_102)
         {
            t = getTimer();
            ...
            {
               if(this.var_607[priority] < priority)
               {
                  ++this.var_607[priority];
                  skip = true;
               }
            }
            if(!skip)
