package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1320:uint = 1;
      ...
      public function addEventListener(param1:String, param2:Function, param3:Boolean = false, param4:int = 0, param5:Boolean = false) : void
      {
         ...
         else
         {
            ...
            {
               if(_loc8_.callback == param2 && _loc8_.var_1991 == param3)
               {
                  return;
               }
      ...
      public function removeEventListener(param1:String, param2:Function, param3:Boolean = false) : void
      {
         ...
         if(!this._disposed)
         {
            _loc4_ = this.var_253[param1];
            if(_loc4_)
            {
               _loc5_ = 0;
               for each(_loc6_ in _loc4_)
               {
                  if(_loc6_.callback == param2 && _loc6_.var_1991 == param3)
                  {
                     _loc4_.splice(_loc5_,1);
                     _loc6_.callback = null;
                     if(_loc4_.length == 0)
                     {
                        delete this.var_253[param1];
                     }
                     return;
                  }
                  _loc5_++;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!this._disposed)
         {
            ...
            if(this._result == const_938)
            {
               if(this.var_582 != null)
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ": " + this.var_582.message,this.var_582.errorID,this.var_582);
               }
               else
               {
                  Core.crash("Error catched when handling " + getQualifiedClassName(param1) + ". No error data available!",0,this.var_582);
               }
            }
      ...
      {
         return !!this._disposed ? false : this.var_253[param1] != null;
      }
      
      public function callEventListeners(param1:String) : void
      {
         var _loc3_:* = null;
         var _loc2_:Array = this.var_253[param1];
         if(_loc2_)
         {
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = this.var_253[event.type];
         if(array)
         {
            callbacks = [];
         ...
         this._result = !!event.isDefaultPrevented() ? uint(const_1320) : uint(const_599);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!this._disposed)
         {
            ...
            {
               _loc2_ = this.var_253[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete this.var_253[_loc1_];
            }
            this.var_253 = null;
