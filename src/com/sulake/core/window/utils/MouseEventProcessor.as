package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected var var_1509:IMouseCursor;
      ...
      {
         super();
         this.var_655 = new Point();
         this.var_1509 = param1;
         if(var_324 == null)
         {
            var_324 = new Array();
            var_324[0] = MouseCursorType.const_314;
            var_324[1] = MouseCursorType.const_31;
            var_324[2] = MouseCursorType.const_314;
            var_324[3] = MouseCursorType.const_314;
            var_324[4] = MouseCursorType.const_314;
            var_324[5] = MouseCursorType.const_31;
            var_324[6] = MouseCursorType.const_314;
         }
         if(var_280 == null)
         {
            var_280 = new Array();
            var_280[0] = WindowState.const_108;
            var_280[1] = WindowState.const_85;
            var_280[2] = WindowState.const_96;
            var_280[3] = WindowState.const_69;
            var_280[4] = WindowState.const_53;
            var_280[5] = WindowState.const_99;
            var_280[6] = WindowState.const_83;
         }
      }
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_74 = state.var_1238 as WindowController;
         this.var_168 = state.var_1234 as WindowController;
         this.var_152 = state.renderer;
         this.var_810 = state.var_1235;
         eventQueue.begin();
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_655.x || event.stageY != this.var_655.y)
            {
               this.var_655.x = event.stageX;
               this.var_655.y = event.stageY;
               array = new Array();
               this.var_139.groupParameterFilteredChildrenUnderPoint(this.var_655,array,WindowParam.const_30);
            }
            index = array != null ? int(array.length) : 0;
            this.var_1509.type = MouseCursorType.const_31;
            if(index == 0)
            {
               if(event.type == MouseEvent.MOUSE_MOVE)
               {
                  if(this.var_74 != this.var_139 && !this.var_74.disposed)
                  {
                     this.var_74.getGlobalPosition(point);
                     this.var_74.update(this.var_74,new WindowMouseEvent(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,this.var_74,null,event.stageX - point.x,event.stageY - point.y,event.stageX,event.stageY,event.altKey,event.ctrlKey,event.shiftKey,event.buttonDown,event.delta));
                     this.var_74 = WindowController(this.var_139);
                  }
               }
            }
            while(--index > -1)
               ...
               if(child != null && child.visible)
               {
                  if(event.type == MouseEvent.MOUSE_MOVE)
                  {
                     if(child != this.var_74)
                     {
                        ...
                        {
                           this.var_74.getGlobalPosition(point);
                           this.var_74.update(this.var_74,new WindowMouseEvent(WindowMouseEvent.WINDOW_EVENT_MOUSE_OUT,this.var_74,child,event.stageX - point.x,event.stageY - point.y,event.stageX,event.stageY,event.altKey,event.ctrlKey,event.shiftKey,event.buttonDown,event.delta));
                        }
                        if(!child.disposed)
                        {
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_74 && !this.var_74.disposed)
                     {
                        ...
                        {
                           this.var_810.mouseEventReceived(event.type,this.var_74);
                        }
                     }
                  }
                  temp = child.parent;
                  ...
                  if(this.var_74 is IInteractiveWindow)
                  {
                     try
                     {
                        this.var_1509.type = getMouseCursorByState(this.var_74.state);
                     }
                     catch(e:Error)
                     {
                        var_1509.type = MouseCursorType.const_31;
                     }
                  }
         ...
         state.var_1238 = this.var_74;
         state.var_1234 = this.var_168;
         state.renderer = this.var_152;
         state.var_1235 = this.var_810;
      }
      ...
      private function passMouseEvent(param1:WindowController, param2:MouseEvent, param3:Boolean = false) : WindowController
      {
         ...
         if(param1.testParamFlag(WindowParam.const_473))
         {
