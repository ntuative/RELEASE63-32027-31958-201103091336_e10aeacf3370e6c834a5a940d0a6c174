package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1539:int = 4;
      ...
      {
         return this.var_423.length + this.var_279.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_279.length < const_1539 && this.var_423.length > 0)
            {
               _loc1_ = this.var_423.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_423.indexOf(loader);
            if(index > -1)
            {
               this.var_423.splice(index,1);
            }
            index = this.var_279.indexOf(loader);
            if(index > -1)
            {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_423)
            {
               if(param2)
               {
