package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1683:uint = 1;
      
      protected static const const_1682:uint = 2;
      
      protected static const const_1684:uint = 4;
      
      protected static const const_1685:uint = 3;
      
      protected static const name_3:uint = 7;
      
      public static const const_1595:int = 5;
      
      public static const LIBRARY_LOADER_FINALIZE:String = "LIBRARY_LOADER_FINALIZE";
      
      public static const const_1182:Boolean = false;
      
      public static const const_1539:int = 2;
      
      private static var var_876:Array = new Array();
      
      private static var var_716:Array = new Array();
      ...
      protected var var_232:URLRequest;
      
      protected var var_783:XML;
      ...
      protected var var_1004:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_1003:Boolean = false;
      
      protected var var_2772:String = "";
      
      protected var var_2771:String = "";
      ...
      protected var var_2860:uint = 0;
      
      protected var var_2773:Boolean = true;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_42.load(param1.var_232,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1182)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_716[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_716.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_716.length < const_1539 && false)
            {
               _loc1_ = var_876.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_716.push(_loc1_);
                  _loc1_.var_42.load(_loc1_.var_232,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_232 ? this.var_232.url : null;
      }
      ...
      {
         return !!this.var_1004 ? uint(this.var_1253 - this.var_2011) : uint(getTimer() - this.var_2011);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_385,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_42.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_232.url);
      ...
      {
         if(this.var_1003 && !_disposed)
         {
            this.var_1003 = false;
            if(!this.var_1004 && this.var_232)
            {
               this.var_1253 = -1;
               this.var_2011 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1004 && !_disposed && this.var_1252 > 0)
         {
            try
            {
               this.var_42.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_232,const_1595 - this.var_1252);
            if(this.var_2773)
            {
               this.var_232.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_232.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2773 = false;
            }
            --this.var_1252;
            this.var_448 = 0;
            this.var_42.load(this.var_232,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               ...
               this.debug("Load event STATUS " + this.var_385 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_385,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_385,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_385))
               {
                  this.var_1253 = getTimer();
            ...
            case TimerEvent.TIMER:
               ...
               if(!_disposed)
               {
                  this.var_448 = LibraryLoader.name_3;
         ...
         if(this.var_448 == LibraryLoader.const_1685)
         {
         ...
         if(this.var_448 == LibraryLoader.name_3)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_385,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         if(this.var_636 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_232.url + "!");
            }
            return false;
         }
         try
         ...
         try
         {
            this.var_783 = new XML(bytes.readUTFBytes(bytes.length));
         }
         catch(e:Error)
         {
            if(!retry())
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_385,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_385,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
